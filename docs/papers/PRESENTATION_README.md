...
Run the comprehensive test analysis from the repository root:
```bash
...
The paper is written in LaTeX format: `docs/papers/cvpr_workshop_paper.tex`
...
Open the presentation: `docs/papers/presentation_slides.html`

The slides are built with Reveal.js and can be:
1. Opened directly in a browser
2. Served locally from `docs/papers`: `python -m http.server 8000` then navigate to http://localhost:8000/presentation_slides.html
...
- Paper: See `docs/papers/cvpr_workshop_paper.tex`
- Slides: See `docs/papers/presentation_slides.html`
